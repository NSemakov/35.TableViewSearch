...
NSOperation реализована в методе. минус - в блоке не поменять переменную извне. т.е. чтобы в методе использовать операции и отрисовать UI надо держать элемент UI в проперти. Также важно помнить, что если будет проверяться флаг isCancelled, то нужно создать weak ссылку на операцию и уже делать if (weakOperation.isCancelled).
